...
def preorder_reconstruct(node, resource_list, resource_type, parent=None):
    ...
    new_node = Node(name=node.name, category_name=node.category_name, resources=resources, resource_type=resource_type,
                    parent=parent)
    if new_root is None:
        new_root = new_node
...
def has_resource(node):
    ...
    if Video.objects.filter(category_id=cat_id) or Audio.objects.filter(category_id=cat_id) or Text.objects.filter(
            category_id=cat_id):
        return True
    return False
...
def wc_search_resource(request):
    ...
    categories = [{"id": item.id, "category_name": item.name} for item in
                  Category.objects.filter(name__contains=keyword)]
    new_cats = []
